...
public class StorageImpl<K, V> implements Storage<K, V> {
    ...
    public void put(K key, V value) {
        // what assert is ??????System.out.println(Arrays.toString(storageMap) + " all arr");
    ...
    private Pair<K, V> findValue(K key) {
        for (int i = 0; i < size; i++) {
            if (storageMap[i].getKey() != null && storageMap[i].getKey().equals(key) && storageMap[i].getValue() != null) {
                return storageMap[i];
            }
